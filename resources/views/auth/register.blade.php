...
<form method="POST" action="{{ route('account.register') }}" class="row  m-auto py-5 form-group text-start">
    @csrf
      <div class="col-10 mb-4">
        ...
        <input id="name" type="text" class="col-7 {{ $errors->has('name') ? ' is-invalid' : '' }}" placeholder="Name" name="name" value="Name {{ old('name') }}" required >
      </div>
      <div class="col-10 mb-4">
        <label for="email" class="col-3">Email Address</label>
        <input id="email" type="email" class="col-7 {{ $errors->has('email') ? ' is-invalid' : '' }}" placeholder="Email address" name="email" value="Email address {{ old('Email address') }}" required>
      </div>
      <div class="col-10 mb-4">
      ...
      <div class="col-10 mb-4">
         ...
         <input id="password-confirm" type="password" class="col-7" placeholder="Password confirm" name="password_confirmation" value="Password"  autocomplete="new-password" required>
      </div>  
